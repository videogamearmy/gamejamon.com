...
class LoginWithAjax {
	...
	public static function login(){
		...
		if( !empty($_REQUEST['log']) && !empty($_REQUEST['pwd']) && trim($_REQUEST['log']) != '' && trim($_REQUEST['pwd'] != '') ){
			...
			if ( strtolower(get_class($loginResult)) == 'wp_user' ) {
				...
				$return['result'] = true;
				$return['message'] = __("Login Successful, redirecting...", MAHA_TEXT_DOMAIN);
				...
				if( !empty(self::$data['no_login_refresh']) && self::$data['no_login_refresh'] == 1 ){
					...
					$return['message'] = __("Login successful, updating...", MAHA_TEXT_DOMAIN);
				}
				...
				//Undefined Error
				$return['result'] = false;
				$return['error'] = __('An undefined error has ocurred', MAHA_TEXT_DOMAIN);
			}
		}else{
			$return['result'] = false;
			$return['error'] = __('Please supply your username and password.', MAHA_TEXT_DOMAIN);
		}
		$return['action'] = 'login';
	...
	public static function register(){
		...
		if ( !is_wp_error($errors) ) {
			//Success
			$return['result'] = true;
			$return['message'] = __('Registration complete. Please check your e-mail.', MAHA_TEXT_DOMAIN);
	...
	public static function remember(){
		...
		if ( $result === true ) {
			...
			$return['result'] = true;
			$return['message'] = __("We have sent you an email", MAHA_TEXT_DOMAIN);
