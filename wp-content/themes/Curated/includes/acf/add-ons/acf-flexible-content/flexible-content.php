...
class acf_field_flexible_content extends acf_field
{
	...
	*/
	...
	function input_admin_enqueue_scripts()
	{
		...
		wp_register_script( 'acf-input-flexible-content-cookie', $this->settings['dir'] . 'js/jquery.cookie.js', array(), $this->settings['version']);
		...
		<div class="acf_flexible_content">
			...
			<div class="clones">
				...
				<div class="layout" data-layout="<?php echo $layout['name']; ?>">
					...
					<p class="menu-item-handle"><span class="fc-layout-order"> <?php echo $layout['label']; ?></p>
			...
			<div class="values">
				...
				$all_fields_value = "";
				if($field['value']):
					$all_fields_value = $field['value'];

					$a = 0;
					foreach($all_fields_value as $fv => $fv_value):
						$new_arr_fvv = array_values($fv_value);
						// echo $new_arr_fvv[1];
						if ( is_array($new_arr_fvv) ) {

							if ( $new_arr_fvv[1] == 1 ) {
								$all_box_full[$a] = $fv_value;
							}
							if ( $new_arr_fvv[1] == 2 ) {
								$all_box_content[$a] = $fv_value;
							}
				...
				<div class="area box-full active">
					<?php _e("This is <strong>Full Width Area</strong>. You can put components like Slider here. ",'acf'); ?>
					<?php
					if ($all_box_full):
						// BOX FULL LOOP ++++++++++++++++++++++++++++++++++
						// foreach($field['value'] as $i => $value):
						foreach($all_box_full as $bf => $bf_value):
						
							// validate layout
							if( !isset($layouts[$bf_value['acf_fc_layout']]) )
							{
								continue;
							}
							...
							// vars
							$bf_layout = $layouts[$bf_value['acf_fc_layout']]; 
							
							
							?>
							<div class="layout" data-toggle="closed" data-layout="<?php echo $bf_layout['name']; ?>">
								
								<input type="hidden" name="<?php echo $field['name'] ?>[<?php echo $bf ?>][acf_fc_layout]" value="<?php echo $bf_layout['name']; ?>" />
								
								<a class="ir fc-delete-layout" href="#"></a>
								<p class="menu-item-handle"><span class="fc-layout-order"> <?php echo $bf_layout['label']; ?></p>
								
								
								<table class="widefat acf-input-table <?php if( $bf_layout['display'] == 'row' ): ?>row_layout<?php endif; ?>">
								<?php if( $bf_layout['display'] == 'table' ): ?>
									<thead>
										<tr>
											<?php foreach( $bf_layout['sub_fields'] as $bf_sub_field_i => $bf_sub_field): 
												
												// add width attr
												$bf_attr = "";
												
												if( count($bf_layout['sub_fields']) > 1 && isset($bf_sub_field['column_width']) && $bf_sub_field['column_width'] )
												{
													$bf_attr = 'width="' . $bf_sub_field['column_width'] . '%"';
												}
												
												?>
												<th class="acf-th-<?php echo $bf_sub_field['name']; ?>" <?php echo $bf_attr; ?>>
													<span><?php echo $bf_sub_field['label']; ?></span>
													<?php if( isset($bf_sub_field['instructions']) ): ?>
														<span class="sub-field-instructions"><?php echo $bf_sub_field['instructions']; ?></span>
													<?php endif; ?>
												</th><?php
											endforeach; ?>
										</tr>
								...
								<tbody>
									...
									foreach( $bf_layout['sub_fields'] as $bf_j => $bf_sub_field ): ?>
										...
										if( $bf_layout['display'] == 'row' ): ?>
											<tr>
												<td class="label">
													<label><?php echo $bf_sub_field['label']; ?></label>
													<?php if( isset($bf_sub_field['instructions']) ): ?>
														<span class="sub-field-instructions"><?php echo $bf_sub_field['instructions']; ?></span>
													<?php endif; ?>
												</td>
										...
										<td>
											...
											// add value
											$bf_sub_field['value'] = isset($bf_value[$bf_sub_field['key']]) ? $bf_value[$bf_sub_field['key']] : false;
											
											// add name
											$bf_sub_field['name'] = $field['name'] . '[' . $bf . '][' . $bf_sub_field['key'] . ']';
											
											// clear ID (needed for sub fields to work!)
											unset( $bf_sub_field['id'] );
										
											// create field
											do_action('acf/create_field', $bf_sub_field);
				...
				<div class="area-x box-sidebar sidebar-off">
					<?php _e("This is just <strong>Sidebar Area</strong>.</br> You can custom the widgets via \"Appearance > Widgets\" ",'acf'); ?>
				</div>

				<div class="area box-content sidebar-off">
					<?php _e("This is <strong>Content Area</strong>. You can put components like Blocked Posts here. ",'acf'); ?>
					<?php
					if ($all_box_content):
						// BOX CONTENT LOOP ++++++++++++++++++++++++++++++++++
						// foreach($field['value'] as $i => $value):
						foreach($all_box_content as $bc => $bc_value):
						
							// validate layout
							if( !isset($layouts[$bc_value['acf_fc_layout']]) )
							{
								continue;
							}
							...
							// vars
							$bc_layout = $layouts[$bc_value['acf_fc_layout']]; 
							
							
							?>
							<div class="layout" data-toggle="closed" data-layout="<?php echo $bc_layout['name']; ?>">
								
								<input type="hidden" name="<?php echo $field['name'] ?>[<?php echo $bc ?>][acf_fc_layout]" value="<?php echo $bc_layout['name']; ?>" />
								
								<a class="ir fc-delete-layout" href="#"></a>
								<p class="menu-item-handle"><span class="fc-layout-order"> <?php echo $bc_layout['label']; ?></p>
								
								
								<table class="widefat acf-input-table <?php if( $bc_layout['display'] == 'row' ): ?>row_layout<?php endif; ?>">
								<?php if( $bc_layout['display'] == 'table' ): ?>
									<thead>
										<tr>
											<?php foreach( $bc_layout['sub_fields'] as $sub_field_i => $sub_field): 
												
												// add width attr
												$attr = "";
												
												if( count($bc_layout['sub_fields']) > 1 && isset($sub_field['column_width']) && $sub_field['column_width'] )
												{
													$attr = 'width="' . $sub_field['column_width'] . '%"';
												}
								...
								<tbody>
									...
									foreach( $bc_layout['sub_fields'] as $j => $sub_field ): ?>
										...
										<td>
											...
											// add value
											$sub_field['value'] = isset($bc_value[$sub_field['key']]) ? $bc_value[$sub_field['key']] : false;
											
											// add name
											$sub_field['name'] = $field['name'] . '[' . $bc . '][' . $sub_field['key'] . ']';
			...
			<ul class="hl clearfix flexible-footer">
				<li class="right">
					<a href="javascript:;" class="add-row-end acf-button button button-primary button-large"><?php echo $button_label; ?></a>
					<div class="acf-popup">
						<ul>
