...
class acf
{
	...
	*/
	...
	function include_before_theme()
	{
		...
		include_once('core/fields/sidebar_selector-v4.php');
        if(function_exists('register_field')){
            register_field('acf_field_sidebar_selector', dirname(__File__) . 'core/fields/sidebar_selector-v3.php');
        }
 ...
 */
function maha_remove_acf_menu(){
    // get the current user
    $current_user = wp_get_current_user();
    // match and remove if needed
    if( !in_array( $current_user->user_login, array() ) ){
        remove_menu_page('edit.php?post_type=acf');
    }
}
add_action( 'admin_menu', 'maha_remove_acf_menu', 999 );
