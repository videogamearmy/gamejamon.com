...
class ReduxFramework_typography {
    ...
    function __construct($field = array(), $value = '', $parent) {
        ...
        $this->google_html = ReduxFramework::$_dir . 'inc/fields/typography/googlefonts.html';
        $this->google_html_url = ReduxFramework::$_url . 'inc/fields/typography/googlefonts.html';
        $this->google_html_php = ReduxFramework::$_dir . 'inc/fields/typography/googlefonts_html.php';
        $this->google_json = ReduxFramework::$_dir . 'inc/fields/typography/googlefonts.json';
        $this->google_json_url = ReduxFramework::$_url . 'inc/fields/typography/googlefonts.json';
        $this->google_json_php = ReduxFramework::$_dir . 'inc/fields/typography/googlefonts_json.php';
    ...
    function render() {
        ...
        if ($this->field['font-family'] === true) {
            ...
            if (($this->field['google'] == true) || ($this->field['custom_fonts'] !== false && !empty($this->field['custom_fonts']))) {
            ...
            if ($this->field['google'] == true ) {
                ...
                //} else if (file_exists(ReduxFramework::$_dir . 'inc/fields/typography/googlefonts.html')) {
                    ...
                    if (empty($googleHTML)) {
                        $googleHTML = Redux_Helpers::curlRead($this->google_html_url);

                        $pattern = preg_quote('Webfonts', '/');
                        $pattern = "/^.*$pattern.*\$/m";
                        if(preg_match_all($pattern, $googleHTML, $matches)){
                            // Nothing to do
                        } else {
                            include $this->google_html_php;
                            $googleHTML = $gwf_html;
                        }
                    }
        ...
        if (!isset($this->field['preview']) || $this->field['preview'] !== false):
            if (isset($this->field['preview']['text'])) {
                $g_text = $this->field['preview']['text'];
                 ...
                 $g_text = '0123456789</br> ABCDEFGHIJKLMNOPQRSTUVWXYZ</br> abcdefghijklmnopqrstuvwxyz';
            }
    ...
    function getGoogleArray() {
        ...
        if (!isset($this->parent->fonts['google']) || empty($this->parent->fonts['google'])) {
            ...
            // Fallback if file_get_contents won't work for wordpress. MEDIATEMPLE
            if (empty($fonts)) {
                // $fonts = json_decode(Redux_Helpers::curlRead($this->google_json_url), true);

                // $pattern = preg_quote('Oswald', '/');
                // $pattern = "/^.*$pattern.*\$/m";
                // if(preg_match_all($pattern, $fonts, $matches)){
                    // Nothing to do
                // } else {
                    include $this->google_json_php;
                    $fonts = json_decode($gwf_json, true);
                // }
            }
    ...
    function getGoogleFonts() {
        ...
        //$wp_filesystem->put_contents(ReduxFramework::$_dir . 'inc/fields/typography/googlefonts.html', $gfonts, FS_CHMOD_FILE
