...
$key , $key .site-background, $key .first-quote,  $key .related_image_wrap, $key .gravatar img, $key .comment-reply-link, $key .inner_slide .numeric_controls a, $key .hr_content, $key .news-thumb, $key .post-format-icon, $key.ajax_controlls a, $key #js_sort_items a, $key .portfolio-sort-container .inner-entry, $key .inner_slide{
background-color:$bg;
color: $color;
}
...
$key .meta-color, $key .sidebar, $key .sidebar a, $key .minor-meta, $key .minor-meta a, $key .text-sep, $key .quote-content, $key .quote-content a, $key blockquote, $key .post_nav a, $key .comment-text, $key .template-search  a.news-content, $key .subtitle_intro, $key div .hr_content, $key .hr a, $key .breadcrumb, $key .breadcrumb a, $key .portfolio_excerpt, $key .avia-big-box-bellow, $key .side-container-inner, $key .news-time, div $key #js_sort_items a{
color: $meta;
}

$key a, $key .widget_first, $key #js_sort_items a.active_sort{
color:$primary;
}

$key a:hover, $key h1 a:hover, $key h2 a:hover, $key h3 a:hover, $key h4 a:hover, $key h5 a:hover, $key h6 a:hover,  $key .template-search  a.news-content:hover, $key #js_sort_items a:hover{
color: $secondary;
}	

$key .primary-background, $key .dropcap2, $key .primary-background a, $key .slide_controls a:hover, $key .avia_welcome_text, $key .avia_welcome_text a, $key .pagination a:hover, div $key .button, $key #submit, $key .big_button, $key .iconbox_icon, $key .iconbox_top_icon,  #top $key .active_item, $key .contentSlideControlls .activeItem, $key input[type='submit'], $key .menu_pointer, $key .callout.hero-text .content-area p a{
background-color: $primary;
color:$bg;
border-color:$primary;
}

$key .button:hover, $key.ajax_controlls a:hover, $key #submit:hover, $key .big_button:hover, $key .contentSlideControlls a:hover, $key input[type='submit']:hover, $key .callout.hero-text .content-area p a:hover{
...
$key .iconbox_top,  $key .pullquote_boxed, $key.body_ajax, $key.click_ping, $key.body_ajax_fake, $key .related_image_wrap{
...
#top $key.title_container .main-title, #top $key.title_container .main-title a, #top $key .title_container .main-title, #top $key .title_container .main-title a, $key .slideshow li, #top $key .callout.hero-text .content-area{
color:$meta;
}
...
";
	...
	switch($key)
	{
		...
		$output .= " 
			...
			#header $key.container_wrap { border-color: $primary; }
			";
...
$avia_config['style'] = array(
		...
		//google webfonts
		array(
		'elements'	=> 'body, .flex_column h1, .flex_column h2, .flex_column h3, .flex_column h4, .flex_column h5, .flex_column h6, #top .widgettitle, .iconbox_content_title, .iconbox_top_content_title',
		'key'	=>	'google_webfont',
