...
class avia_dynamic_template
{
	...
	*/
	...
	{
		if(isset($this->template_elements[$array_key]) && isset($this->template_elements[$array_key]['saved_value'])) { 
...
*/
	...
	*/
	...
	function hr($element)
	{
		...
		switch($dynamic_hr)
		{
			...
			case 'dotted': 			$output .= avia_advanced_hr(false ,'hr_dotted '.$extraClass); break;
	...
	*/
	...
	{	
		...
		//check for second partner list in a row
		if($this->check($this->dynamic_counter - 2) == "logo"){ $extraClass .= " addon_list ";}
		if($this->check($this->dynamic_counter ) == "logo"){ $extraClass .= " start_list ";}
	...
	*/
	...
	function page_split($element)
	{
		...
		$extraClass = "dynamic_element dynamic_el_".$this->dynamic_counter. " ".avia_is_dark_bg($page_split_style, true);
		$output = "";
		$output .= "</div>";
...
*/
		...
		$avia_config['page_split_style'] = $page_split_style;
	...
	*/
	...
	{	
		...
		switch($dynamic_heading_type)
		{
			...
			case 'custom': 		$heading = $dynamic_heading_custom; break; //$subtitle =	$dynamic_subtitle_custom;  
		}
		
		if($dynamic_heading_meta != 'yes')
		{
			$bc = false;
		}
		...
		if(isset($heading))
		{
			$args = array('title'=>$heading, 'subtitle'=>$subtitle, "class" => "stretch_full dynamic_element dynamic_el_".$this->dynamic_counter, 'breadcrumb' => $bc);
			
			if($dynamic_heading_type == 'custom') $args['link'] = false;
			
			$output = avia_title($args);
		}
	...
	*/
	...
	if ($avia_config['layout']['current']['sidebar'] != "hidden") $avia_config['dynamic_portfolio'] = true;
